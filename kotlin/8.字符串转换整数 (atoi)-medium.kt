...
class Solution {
    fun myAtoi(s: String): Int {
        ...
        val sign = if(charArray[0] == '-') -1 else 1
        var index = if(charArray[0] == '-' || charArray[0] == '+') 1 else 0
        val max = Int.MAX_VALUE / 10
        var result = 0
        while(index < charArray.size){
            if(charArray[index].isDigit().not()) break
            if(result > max || (result == max && charArray[index] > '7')) {
