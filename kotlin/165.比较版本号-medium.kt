/*
 * @lc app=leetcode.cn id=165 lang=kotlin
 *
 * [165] 比较版本号
 *
...
class Solution {
    fun compareVersion(version1: String, version2: String): Int {
        val v1:Int = version1.length
        val v2:Int = version2.length
        var i:Int = 0
        var j:Int = 0
        while (i < v1 || j < v2) {
            var x:Int = 0;
            while(i < v1 && version1[i] != '.'){
                x = x * 10  - '0'.toInt() + version1[i].toInt()
                ++i
            }
            ++i
            var y:Int = 0
            while(j < v2 && version2[j] != '.'){
                y = y * 10  - '0'.toInt() + version2[j].toInt()
                ++j
            }
            ++j
            if(x != y){
